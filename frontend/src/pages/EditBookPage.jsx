 ...
 const handleExportPpt = async () => {
  ...
  try {
    const response = await axiosInstance.get(
      `${API_ENDPOINTS.EXPORTS.PPT}/${bookId}/pptx`, // 👈 ensure correct route
      {
        responseType: "blob",
        validateStatus: (status) => status < 500, // allow backend error bodies
      }
    );
    ...
    // ✅ Safe filename (Windows compatible)
    const safeName = (book.title || "book").replace(/[\\/:*?"<>|]/g, "");
